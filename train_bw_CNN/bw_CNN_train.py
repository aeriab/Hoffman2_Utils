...
### New code for black and white CNN ###

def get_subset(mmap_array, prop):
    num_samples = int(mmap_array.shape[0] * prop)
    # Slice [:, :, :, 0:1] to keep only the genotype channel
    # Using 0:1 keeps the shape as (N, 354, 120, 1) instead of (N, 354, 120)
    return mmap_array[:num_samples, :, :, 0:1]
