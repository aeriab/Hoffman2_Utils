...
parser.add_argument('model_json', type=str, help="Path to model architecture JSON file")
parser.add_argument('weights_h5', type=str, help="Path to model weights file")
parser.add_argument('image_npy', type=str, help="Path to colored .npy file")
parser.add_argument('--output', type=str, default='predictions.txt', help="Output filename")
...
# Use mmap_mode to avoid loading the entire large file into RAM immediately
...
# Create a tf.data.Dataset to stream data from disk/RAM to GPU in batches.
# This prevents the InternalError caused by trying to allocate the entire 14GB array as a single constant on the GPU.
dataset = tf.data.Dataset.from_tensor_slices(data)
dataset = dataset.batch(args.batch_size).prefetch(tf.data.AUTOTUNE)

# Run inference using the dataset pipeline
prediction_probabilities = model.predict(dataset, verbose=1)
