...
if __name__ == "__main__":
    # Args: <input_txt> <output_npy> <n_samps> <window> <index>
    ...
    # 2. SMART-SAVE: Detect if the Manager wants 1 channel or 2
    # Open the existing memmap in read/write mode
    big_array = np.lib.format.open_memmap(OUTPUT_PATH, dtype=np.float32, mode="r+")

    # If the file has 4 dims: (Total_Sims, Samples, Window, 2)
    if len(big_array.shape) == 4:  
        big_array[INDEX] = mut_array_sorted
    
    # If the file has 3 dims: (Total_Sims, Samples, Window)
    else: 
        # Slice it to take only the first channel (Presence/Absence)
        big_array[INDEX] = mut_array_sorted[:, :, 0]

    del big_array  # Force flush to disk